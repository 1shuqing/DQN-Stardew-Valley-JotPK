...
REPLAY_SIZE = 5000     #回放池
INITIAL_EPSILON = 0.99   #初始探索率
FINAL_EPSILON = 0.05    #最终探索率
EPSILON_DECAY_STEP = 600  #探索率下降所需步数
BATCH = 64
GAMMA = 0.95 #下一动作Q值的权重
L_R = 0.0005
UPDATE_STEP = 500   #目标网络更新频率
EPOCH = 3000    #训练轮数
ACTION_SPACE = 14


class NET(nn.Module):   #神经网络
    def __init__(self,height,width,action_dim,fusion_dim):
        super(NET,self).__init__()
        self.state_w =width
        self.state_h = height
        self.action_dim = action_dim
# 玩家数量*1 + 玩家坐标*4 + 敌人最大数量*坐标-> 20*4=80 + 敌人数量*1 = 86维
        ...
        #全连接层
        self.fc1 = nn.Linear(int((self.state_w/4)*(self.state_h/4)*64)+self.fusion_dim,512)
    ...
    #前向传播
    def forward(self,screen_x,fusion_x):
        x =self.cnn(screen_x)
        x = x.reshape(x.size(0),-1)
        fusion_x = fusion_x.squeeze(1)
...
class DQN():
    def __init__(self,height,width,action,fusion_dim,model,log,load_replay_buffer=True):
        self.model = model
        self.log = log
        #log用
        self.loss_history = []
        ...
        self.target_net = NET(height,width,action,fusion_dim)
        self.target_net.to("cuda")
        self.eval_net = NET(height,width,action,fusion_dim)
    ...
    def choose_action(self,screen_state,fusion_state):
        #epsilon贪心算法随机探索
        ...
        if random.random() <= self.epsilon:
            ...
            fusion_state = fusion_state.to("cuda", dtype=torch.float).unsqueeze(0)
            # 根据剩余时间和yolo检测结果判断Q值最大的动作
            Q = self.eval_net(screen_state,fusion_state)    #eval_net返回一个存有各动作Q值的张量

            self.last_q = Q.cpu().detach().numpy().flatten()
            return torch.argmax(Q).item()  # 检索结果转换成python int

    def replay_data(self,screen_state,fusion_state,action,reward,n_screen_state,n_fusion_state,done):
        #动作转one-hot编码
        ...
        self.replay_buffer.append(
            (screen_state.cpu(),fusion_state.cpu(),oh_action,reward,n_screen_state.cpu(),n_fusion_state.cpu(),done)
        )
    ...
    def train(self):
        ...
        screen_batch, fusion_batch, oh_action_batch, reward_batch, n_screen_batch, n_fusion_batch, done_batch = zip(*batch)
        #拆分batch数据
        screen_batch = torch.stack(screen_batch).to("cuda", dtype=torch.float)
        fusion_batch = torch.stack(fusion_batch).to("cuda", dtype=torch.float)
        ...
        n_fusion_batch = torch.stack(n_fusion_batch).to("cuda", dtype=torch.float)
        done_batch =  torch.tensor(done_batch, dtype=torch.bool).to("cuda")

        # target_net 计算下一状态Q值
        n_Q_batch = self.target_net(n_screen_batch,n_fusion_batch).detach()
        Q_target=[]
        ...
        # eval_net 计算当前状态Q值
        Q_batch = self.eval_net(screen_batch,fusion_batch)
...
#加载yolo
yolo_model = YOLO(r"runs/detect/train4/weights/best.pt")
...
def get_game_state():
    ...
    screen_r = cv2.resize(gray, (96, 88))
    ...
    fusion_tensor = torch.cat([yolo_tensor, time_tensor], dim=1)
    return screen_tensor, fusion_tensor, screen
...
def c_reward(p_enemy_count,enemy_count,alive,finish,action,player_box,enemy_boxes,time_left):
    ...
    if alive and enemy_count < p_enemy_count:
        reward += 1.0*(p_enemy_count - enemy_count)
    ...
    if finish:
        ...
        # print("通关+5分")
    #场上无敌人
    if alive and enemy_count and time_left<1 == 0:
        reward+= 0.3
        # print("清场+0.3分")
    # 原地不动惩罚
...
if __name__ == '__main__':
    ...
    model = DQN(88,96,action=ACTION_SPACE,fusion_dim=86,model=model_path,log=log_path,load_replay_buffer=load_replay_buffer)
    if load_model:
        model.load_model()
    ...
    while len(model.replay_buffer) < BATCH:  # 填充经验池直到能启动train
        ...
        time_left = 1
        while time_left>=1:
            screen_state, fusion_state, screen = get_game_state()
            _,_,enemy_count,_,_,time_left = check(screen)
        screen_state, fusion_state, screen = get_game_state()
        _, _, enemy_count, _, _, time_left = check(screen)
        action = random.randint(0, ACTION_SPACE-1)  # 纯随机探索
        move(action)
        n_screen_state, n_fusion_state,n_screen = get_game_state()
        ...
        model.replay_data(screen_state, fusion_state, action, reward, n_screen_state, n_fusion_state, done)
        if done:
            c_action.release_all_key()
            restart()
            screen_state, fusion_state,screen = get_game_state()
            _,_,enemy_count,_,_,_ = check(screen)
    print("经验池填充完成，开始训练")
    ...
    for e in range(EPOCH):
        ...
        while not done:
            ...
            steps += 1
            #选动作
            action = model.choose_action(screen_state,fusion_state)
            #做动作
            move(action)
            #获取新状态
            n_screen_state,n_fusion_state,n_screen = get_game_state()
